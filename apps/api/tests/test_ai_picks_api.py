...
def test_ai_picks_api_metadata_data_mode_and_request_id() -> None:
    ...
    assert all(item["home_team"] == "Lincoln Red Imps" for item in data["items"])
    assert all(item["away_team"] == "Inter Club d'Escaldes" for item in data["items"])
    assert all(item["kickoff_at"] == "2026-07-07T16:00:00Z" for item in data["items"])
