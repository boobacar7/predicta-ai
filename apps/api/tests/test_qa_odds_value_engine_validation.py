...
def make_value_service(
    *snapshots: OddsSnapshot,
    predictions: StaticPredictionService | MissingPredictionService | None = None,
...
@pytest.mark.xfail(
    ...
    reason="BLOCKER: exact Decimal equality rejects valid rounded no-vig simplexes.",
)
...
def test_valid_markets_must_not_fail_no_vig_normalization(
    ...
    normalized, _ = calculator.no_vig_probabilities(odds)
    assert sum(normalized.values(), Decimal(0)) == pytest.approx(Decimal(1))
...
@pytest.mark.xfail(
    strict=True,
    reason="HIGH: returned prediction cutoff is trusted even when it exceeds the requested cutoff.",
)
...
@pytest.mark.xfail(
    strict=True,
    reason="HIGH: persisted history is not filtered by active provider source or data_mode.",
)
...
@pytest.mark.xfail(
    strict=True,
    raises=OddsUnavailableError,
    reason="HIGH: provider failure prevents historical reads even when eligible odds are persisted.",
)
...
@pytest.mark.xfail(
    strict=True,
    raises=IncompleteOddsMarketError,
    reason="HIGH: latest incomplete snapshot masks an earlier complete snapshot despite the OpenAPI contract.",
)
...
@pytest.mark.xfail(
    strict=True,
    raises=OddsTemporalLeakageError,
    reason="HIGH: RFC3339 serialization drops microseconds and breaks exact PIT replay.",
)
...
@pytest.mark.xfail(
    strict=True,
    reason="BLOCKER: migration applies data_mode NOT NULL without backfilling valid legacy NULL rows.",
)
...
def test_invalid_probability_ranges_and_simplex_are_refused() -> None:
    ...
    for prediction in (invalid_range, invalid_simplex):
        with pytest.raises(InvalidPredictionError):
            make_value_service(
                make_snapshot(),
                predictions=InvalidPredictionService(prediction),  # type: ignore[arg-type]
            ).evaluate(MATCH_ID, CUTOFF)
...
@pytest.mark.xfail(
    ...
    reason="MEDIUM: missing prediction fields are not converted to InvalidPredictionError.",
)
def test_missing_prediction_metadata_must_be_refused_as_domain_error() -> None:
    ...
    with pytest.raises(InvalidPredictionError):
        make_value_service(
            make_snapshot(),
            predictions=IncompletePredictionService(),  # type: ignore[arg-type]
        ).evaluate(MATCH_ID, CUTOFF)
...
@pytest.mark.xfail(
    strict=True,
    reason="BLOCKER: a valid ordinary market reaches HTTP 500 when Decimal no-vig sum is not exactly 1.",
)
