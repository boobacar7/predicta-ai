...
def write_dataset_artifacts(dataset: MlDataset, json_path: str | Path) -> dict[str, str]:
    path = Path(json_path)
    path.parent.mkdir(parents=True, exist_ok=True)
    payload = dataset.to_dict()
    path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    quality_path = path.with_suffix(".quality.json")
    quality_path.write_text(json.dumps(build_quality_report(dataset), indent=2, sort_keys=True), encoding="utf-8")
